...
var Opened_games_classic = {};
var Playing_now_point = {};
var Opened_games_point = {};
//DUEL
var Playing_now_duel = {};
var Opened_games_duel = {};
var Playing_now_classic_duel = {};
var Opened_games_classic_duel = {};
var Playing_now_point_duel = {};
var Opened_games_point_duel = {};
//
//private
...
var Playing_now_point_private = {};
var Opened_games_point_private = {};
//DUEL PRIVATE
...
var Playing_now_point_private_duel = {};
var Opened_games_point_private_duel = {};
//
var SocketbyName = {}; //  for restrictions on games with 1 account
var Players_name_by_socket_id = {}; //for  sending all the nicks into the room
console.log("working")
////
...
function Win_sequence(socket, xs, ys) {
    ...
    Send_Post_req(UsernamesOfLoosers, '/match_end');
    Send_Post_req(UsernamesOfWinner, '/player_winner_db');
...
function create_lobby(socket, Room, Opened) {
    ...
    if (socket.point == 1) {
        map = [
            ...
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, "Point", null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
            [null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null, null],
...
function Send_Get_req(path, processing) {
    https.get({
            hostname: 'backend.4battle.ru',
            port: 8080,
            path: path,
            method: 'GET',
            headers: {
...
function Send_Post_req(post_data, path) {
...
//PREPEARING DATA  loading maps from db
Send_Get_req('/get_levels', function(res) {
    var bodyChunks = [];
    res.on('data', function(chunk) {
        ...
        for (var i = 0; i < xs.length; i++) {
            Maps[i] = JSON.parse(xs[i].Code)
        }
        console.log(Maps.length + " Maps Loaded")
    })
})
//END OF PREPEARING DATA
//Current online to Go Server
setInterval(function() {
    var AmountOfOnlineUsers = querystring.stringify({
        'online': socketio.engine.clientsCount
    });
    Send_Post_req(AmountOfOnlineUsers, '/online_push')
}, 10000);
//
    ...
    cookieParser(socket.handshake, {}, function(err) {
        ...
        request({
            ...
            if (body == "0") {
                ...
                if (SocketbyName[body] === undefined ) { 
                    var name = body;
...
socketio.on('connection', function(socket) {
    ...
    socket.on('disconnect', function() {
        ...
        if (Playing[socket.room] !== undefined) {
            ...
            Send_Post_req(UsernameofLooser, '/match_end');
    ...
    socket.on('win_detected', function() {
        ...
        if (socket.point == 1) {
            if (
                (Playing[socket.room].board[9][9] == socket.id) &&
                (Playing[socket.room].board[9][10] == socket.id) &&
                (Playing[socket.room].board[9][11] == socket.id) &&
                (Playing[socket.room].board[10][11] == socket.id) &&
                (Playing[socket.room].board[11][11] == socket.id) &&
                (Playing[socket.room].board[11][9] == socket.id) &&
                (Playing[socket.room].board[10][9] == socket.id) &&
                (Playing[socket.room].board[11][10] == socket.id)
            ) {
                console.log("won point")
                xs = [9, 9, 9, 10, 11, 11, 10, 11];
                ys = [9, 10, 11, 11, 11, 9, 9, 10];
    ...
    socket.on('matchmaking_point', function(duel) {
        check(socket);
        socket.UsedForBonus = {}
        socket.bonus_available_for_this_turn = 2;
        socket.bonus = {}
        ...
        socket.bonus[randombonus] = 1;
        socket.emit("get_bonus", randombonus);
        socket.array_play = Playing_now_point
        socket.array_open = Opened_games_point
        var lobbysize = 4;
        socket.mode = "m_point"
        if (duel == 1) {
            socket.array_play = Playing_now_point_duel
            socket.array_open = Opened_games_point_duel
            lobbysize = 2
            socket.mode = "m_point_pvp"
        }
        matchmaking(socket, lobbysize)
    });

    socket.on('private_matchmaking_point', function(roomName, duel) {
        check(socket);
        if ((duel == 1) && (Playing_now_point_private_duel[roomName] !== undefined)) {
            console.log("hijacking detected")
            return
        }
        if ((duel == 0) && (Playing_now_point_private[roomName] !== undefined)) {
            console.log("hijacking detected")
            return
        }
        socket.point = 1;
        ...
        socket.bonus[randombonus] = 1;
        socket.emit("get_bonus", randombonus);
        socket.array_play = Playing_now_point_private
        socket.array_open = Opened_games_point_private
        var lobbysize = 4;
        socket.mode = "p_point"
        if (duel == 1) {
            socket.array_play = Playing_now_point_private_duel
            socket.array_open = Opened_games_point_private_duel
            lobbysize = 2;
            socket.mode = "p_point_pvp"
        }
        private_matchmaking(socket, roomName + socket.mode, lobbysize)
    });
});